...
The homepage hero is a *schematic* depiction of geometric model-based ontology embeddings — not a faithful reproduction of any single method. Its two scenes are drawn after the works cited below (the method keys — **ELEm**, **BoxEL**, **Box²EL**, **HiT**, **OnT** — are the labels shown in the animation itself): the Euclidean box scene after the ℰℒ⁺⁺ box-embedding line of work, and the hyperbolic scene after OnT, which builds on HiT's Poincaré-ball hierarchy encoding. Each of these methods embeds a *single* ontology; the animation takes visual licence in co-embedding two ontologies in one shared space and reading a correspondence off the resulting geometry. Embedding-based ontology matching is itself well established — including hyperbolic variants — but it typically relies on lexical or graph embeddings; to our knowledge these **model-based** geometric embeddings (constructed to be *models* of the ontology's logical axioms) have not yet been used for matching in this shared-space fashion, which would require jointly training over both ontologies (e.g. with bridge mappings) or aligning two per-ontology spaces. So that aspect is suggestive of a direction rather than a depiction of a particular method.
